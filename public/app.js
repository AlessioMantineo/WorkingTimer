(() => {
  ...
  const DAILY_TARGET_BY_WEEKDAY = { 1: 8 * 60, 2: 8 * 60, 3: 8 * 60, 4: 8 * 60, 5: 6 * 60 };
  ...
  function toPublicEntry(row) {
    ...
    return { id: row.id, startAt: row.start_at, endAt: row.end_at, durationMinutes: minutes };
  }

  function assertConfigured() {
    if (!hasSupabaseConfig) throw new Error("Config Supabase mancante: aggiorna public/config.js");
    if (!hasSupabaseRuntime) throw new Error("SDK Supabase non caricato.");
  }
  ...
  function setMode(nextMode) {
    ...
    usernameInput.toggleAttribute("required", isRegister);
    passwordInput.setAttribute("autocomplete", isRegister ? "new-password" : "current-password");
    identifierInput.setAttribute("autocomplete", isRegister ? "email" : "username");
    identifierLabel.textContent = isRegister ? "Email" : "Username o Email";
    identifierInput.placeholder = isRegister ? "nome@email.com" : "username o nome@email.com";
  ...
  function indexAdjustmentsByDate(adjustments) {
    const map = {};
    for (const row of adjustments) map[row.dayDate] = row;
    return map;
  }
  ...
  async function resolveEmailFromIdentifier(identifier) {
    ...
    if (error) throw new Error("Username login non configurato su Supabase (manca funzione SQL).");
    if (!data) throw new Error("Username non trovato.");
    return data;
  }
  ...
  async function createManualEntry(dayDate, fallbackHours) {
    ...
    const { error } = await supabaseClient.from("work_entries").insert({
  ...
  async function saveDayAdjustment(dayDate, dayType, permissionHours, permissionMinutes) {
    ...
    const { error } = await supabaseClient.from("day_adjustments").upsert(
      {
        user_id: currentUser.id,
        day_date: dayDate,
        day_type: dayType,
        permission_minutes: hours * 60 + mins,
        updated_at: new Date().toISOString(),
      },
  ...
  async function resetDay(dayDate) {
    ...
    const [{ error: e1 }, { error: e2 }] = await Promise.all([
      ...
      supabaseClient.from("day_adjustments").delete().eq("user_id", currentUser.id).eq("day_date", dayDate),
    ]);
    if (e1) throw e1;
    if (e2) throw e2;
  }
  ...
  async function renderWeek() {
    ...
    const dayModels = weekDays.map((dayDate) => {
      ...
      const adjustment = adjustmentsByDate[dateKey] || { dayType: "none", permissionMinutes: 0 };
      ...
      return { date: dayDate, dayDate: dateKey, weekday, plannedMinutes, entries: dayEntries, workedMinutes, adjustment, effectiveMinutes };
    });

    const today = new Date();
    const weekWorked = dayModels.reduce((sum, d) => sum + d.effectiveMinutes, 0);
    const underMinimumDays = dayModels.filter((d) => d.effectiveMinutes > 0 && d.effectiveMinutes < DAILY_MINIMUM_MINUTES).length;
      ...
      .map((day) => {
        ...
        return `
          <article class="day-card ${sameDate(day.date, today) ? "today" : ""}">
            <header><h4>${DAY_NAMES[day.weekday]}</h4><p>${day.dayDate}</p></header>
            <div class="day-meta">
            ...
            <div class="day-controls" data-day="${day.dayDate}">
              <label class="compact-control"><span>Stato giorno</span>${dayTypeSelectTemplate(dayType)}</label>
              <label class="compact-control"><span>Permessi ore</span><input type="number" class="perm-hours" min="0" max="12" step="1" value="${permissionHours}" /></label>
              <label class="compact-control"><span>Permessi min</span><input type="number" class="perm-mins" min="0" max="59" step="1" value="${permissionOnlyMins}" /></label>
  ...
  authForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (mode === "register") {
        if (!name || !username) throw new Error("Inserisci nome e username.");
        if (username.length < 3) throw new Error("Username troppo corto (minimo 3).");
        ...
        if (user) {
          await supabaseClient
            .from("profiles")
            .upsert({ id: user.id, name, username, email_login: email }, { onConflict: "id" });
        }
      } else {
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!hasSupabaseRuntime) {
      showMessage("SDK Supabase non caricato.", "error");
      showAuth();
      return;
    }
